...
export function getConsoleScript(collectionRunId?: string): string {
  const runIdLine = collectionRunId ? `var runId = '${collectionRunId}';` : `var runId = null;`;
  const runField = collectionRunId ? `, collection_run_id: runId` : '';
  ...
  var rows = uni.map(function(c) {
    return {
      ...
      rpid: c.rpid || null, collected_by: 'bookmarklet'${runField}
    };
  });
