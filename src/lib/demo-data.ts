...
export function generateDemoProject(realProjectId?: string): { project?: Project; posts: Post[]; comments: Comment[] } {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const post: Post = {
      ...
      view_count: randomInt(1000, 50000),
      comments_count: randomInt(10, 100),
      shares: randomInt(5, 200),
      is_aigc: isAigc,
      aigc_type: isAigc ? randomChoice(['ai_restore', 'ai_image', 'digital_human', 'ai_dub'] as const) : randomChoice(['documentary', 'drama', 'other'] as const),
      narrative_type: randomChoice(NARRATIVE_TYPES),
  ...
  for (let i = 0; i < 5; i++) {
    const post: Post = {
      ...
      view_count: randomInt(10000, 500000),
      comments_count: randomInt(50, 500),
      shares: randomInt(10, 500),
      is_aigc: i < 2,
      aigc_type: i < 2 ? 'ai_restore' : 'documentary',
      narrative_type: randomChoice(NARRATIVE_TYPES),
